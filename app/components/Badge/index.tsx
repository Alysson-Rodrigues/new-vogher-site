...
function Badge({ title, description, imageUrl, badgeBackground, className, onMouseOver, onMouseOut, icons, preventDefault, imageClassName,textClassName }: ServiceBadgeProps) {
    return (
        ...
        >
            <div className={`bg-cover bg-center ${imageClassName} h-[300px]`} style={{ backgroundImage: `url(${imageUrl})` }}></div>
            <div className="p-4">
                <h3 className={`font-bold text-4xl text-[${textClassName}] font-base-neue-bold mb-2`}>{title}</h3>
                <p className={`font-base-neue-regular mb-4 text-2xl ${textClassName}`}>{description}</p>
